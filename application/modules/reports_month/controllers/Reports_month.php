...
/**
 * [ Controller File name : Reports_month.php ]
 */
class Reports_month extends CRUD_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('reports_month/Reports_month_model', 'Reports_month');
		$this->load->model('services/services_model', 'Services');
		$this->load->model('FileUpload_model', 'FileUpload');
		$this->data['page_url'] = site_url('reports_month/reports_month');
		$this->file_allow_type = @array_values($this->file_allow);
		$this->file_allow_mime = @array_keys($this->file_allow);
		$this->file_check_name = '';
		$js_url = 'assets/js_modules/reports_month/reports_month.js?ft=' . filemtime('assets/js_modules/reports_month/reports_month.js');
	...
	{
		$this->session->unset_userdata($this->Reports_month->session_name . '_search_field');
		$this->session->unset_userdata($this->Reports_month->session_name . '_value');
	...
	public function search()
	{
		if (isset($_POST['submit'])) {
			...
			$arr = array($this->Reports_month->session_name . '_search_field' => $search_field, $this->Reports_month->session_name . '_value' => $value);
			$this->session->set_userdata($arr);
		} else {
			$search_field = $this->session->userdata($this->Reports_month->session_name . '_search_field');
			$value = $this->session->userdata($this->Reports_month->session_name . '_value');
		}
		...
		if ($order_by != '') {
			...
			$this->Reports_month->order_field = $field;
			$this->Reports_month->order_sort = $sort;
		}
		$results = $this->Reports_month->read($start_row, $per_page);
		$total_row = $results['total_row'];
		$search_row = $results['search_row'];
		$list_data = $this->setDataListFormat($results['list_data'], $start_row);


		$page_url = site_url('reports_month/reports_month');
		...
		$this->render_view('reports_month/reports_month/list_view');
	}
	...
	public function preview($encrypt_id = "")
	{
		...
		if ($id == "") {
			...
			$results = $this->Reports_month->load($id);
			if (empty($results)) {
				$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				$this->render_view('ci_message/danger');
			} else {
				$this->setPreviewFormat($results);
				$this->render_view('reports_month/reports_month/preview_view');
			}
		}
	}
	...
	{
		$this->data['data_id'] = 0;
		$this->data['member_pro_option_list'] = $this->Reports_month->returnOptionList("tb_promotions", "promotion_id", "promotion_name");
		$this->render_view('reports_month/reports_month/add_view');
	}
	...
	public function save()
	{
		...
		if ($message != '') {
			...
			$id = $this->Reports_month->create($post);
			if ($id != '') {
				...
				$success = FALSE;
				$message = 'Error : ' . $this->Reports_month->error_message;
			}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->data['member_pro_option_list'] = $this->Reports_month->returnOptionList("tb_promotions", "promotion_id", "promotion_name");
				$this->render_view('reports_month/reports_month/edit_view');
			}
		}
	}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			$result = $this->Reports_month->update($post);
			if ($result == false) {
				$message = $this->Reports_month->error_message;
				$ok = FALSE;
			} else {
				$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->Reports_month->error_message;
				$ok = TRUE;
			}
			$json = json_encode(array(
				'is_successful' => $ok,
	...
	public function update_member()
	{
		...
		if ($message != '') {
			...
			$result = $this->Reports_month->update_member($post);
			if ($result == false) {
				$message = $this->Reports_month->error_message;
				$ok = FALSE;
			} else {
	...
	public function del()
	{
		...
		if ($message != '') {
			...
			$result = $this->Reports_month->delete($post);
			if ($result == false) {
				$message = $this->Reports_month->error_message;
				$ok = FALSE;
			} else {
	...
	public function billPDF($billID)
	{
	  ...
	  $bill['bill'] = $this->Reports_month->get_PDF($billID);
