...
var Trainers = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('trainers/trainers/preview/'+ id),
	...
	saveFormData: function () {
		var frm_action = site_url('trainers/trainers/save');
			...
			$.ajax({
				...
				success: function (results) {
					if (results.is_successful) {
						alert_type = "success";
						setTimeout(function(){
							$(window.location).attr('href', site_url('trainers/trainers'));
						}, 1500);
					} else {
						alert_type = "danger";
					}
	...
	saveEdit_Trainer_Form: function(){
		...
		var frm_action = site_url('trainers/trainers/update_trainer');
	...
	saveEditForm: function(){
		$('#editModal').modal('hide');
		var frm_action = site_url('trainers/trainers/update');
		var fdata = $('#formEdit').serialize();
		//fdata += '&edit_remark=' + $('#edit_remark').val();
		fdata += '&' + csrf_token_name + '=' + $.cookie(csrf_cookie_name);
		console.log(fdata);
	...
	deleteRecord: function(){
		var frm_action = site_url('trainers/trainers/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url('trainers/trainers/index/'+ this.current_page));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Trainers.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Trainers.validateFormEdit();
	});//click
	$('#btnSaveEditTrainer').click(function() {
		return Trainers.validateFormEditTrainer();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Trainers.current_page = Math.abs(param_current_page);
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		Trainers.confirmDelete(pProductId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Trainers.deleteRecord();
	});
